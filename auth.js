...
const extractJWT = require('./utils/extract_jwt.js')
...
const auth = ({private_key, public_key, secret, onAdd=() => {}, onDelete=() => {}}) => {
  ...
  const generateRefreshToken = (username, id) => {
    ...
    const refreshToken = Buffer.from(rsa.encryptPrivate(Buffer.from(JSON.stringify(refreshTokenPayload)))).toString('base64')
  ...
  const validateRegisterToken = (register_token) => {
    ...
    const success = (message, rest) => ({ message: message, status: 'success', ...rest })
    const failure = (message, rest) => ({ message: message, status: 'failure', ...rest })
    ...
    try {
      ...
      return failure('decryption failed, encrypted using invalid private key (this could just mean that environments were switched while making the request (local development / production)')
    }

    const data = JSON.parse(Buffer.from(decrypted_data, 'base64').toString('ascii'))

    if(data && data.account_type) {
      return success('successful retreived data; token is valid', data)
    } else {
      return failure('failure while retrieving data; token may be invalid', data)
    }
  }
  ...
  const validateService = (tokenBuffer, service) => {
    let payload = {}
    /*
      payload = {
        secret: <SECRET>,
