...
module.exports = (Logout) => (req, res) => {
  ...
  if(req.body.id || req.body.username) {
    ...
    const requestingUser = req.user   // this is the user making the request
    const proposedChanges = req.body  // these are the changes the user wants to make (req.body.id || req.body.username)
    delete proposedChanges.id         // the id cannot be changed, therefore it is removed here

    const cb = (err, userToBeModified, info) => {
      if(err) return res.status(500).json(info)
      if(requestingUser.account_type === 'admin') db.privilegedModifyUser(userToBeModified.id, proposedChanges, (err, _rtn) => {
      ...
      else db.modifyUser(userToBeModified.id, proposedChanges, (err, _rtn) => {
          if(err) sendFailure(res, err)
